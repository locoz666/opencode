...
test("tree workspace sidebar keeps long session rows truncated and keeps resize handle visible", async ({
  page,
  withProject,
}) => {
  ...
  await withProject(async ({ directory, gotoSession, trackDirectory, trackSession }) => {
    ...
    await expect.poll(async () => await workspaceToggles.count()).toBeGreaterThan(1)
    await workspaceToggles.nth(1).click()
    await expect.poll(async () => await row.count()).toBeGreaterThan(0)
    await expect(row).toBeVisible()
